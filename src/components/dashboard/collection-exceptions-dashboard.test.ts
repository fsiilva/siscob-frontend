...
describe("collection exceptions dashboard UI", () => {
  ...
  it("renderiza cards e filtros de promessa diretamente do contrato", () => { for (const text of ["Promessas para hoje", "Promessas vencidas", "Promessas quebradas", "data.summary.paymentPromiseDueToday", "data.summary.overduePaymentPromise", "data.summary.brokenPaymentPromise", "PAYMENT_PROMISE_DUE_TODAY", "OVERDUE_PAYMENT_PROMISE", "BROKEN_PAYMENT_PROMISE"]) expect(source).toContain(text); });
