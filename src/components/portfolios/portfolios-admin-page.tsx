...
export function PortfoliosAdminPage() {
  ...
  const companies = useMemo(() => {
    ...
    [...context.assignedCompanies, ...context.availableCompanies, ...(context.selectedCompany ? [context.selectedCompany] : [])].forEach((x) => values.set(x.id, x.name));
    return [...values].map(([id, name]) => ({ id: String(id), name })).sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
  }, [context.assignedCompanies, context.availableCompanies, context.selectedCompany]);
  ...
  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    ...
    {editor && <Drawer open title={editor.portfolio ? "Editar Carteira" : "Nova Carteira"} onClose={() => setEditor(null)}><form className="space-y-4 py-5" onSubmit={e => { e.preventDefault(); void save(); }}><label className="block text-sm">Empresa<Select className="mt-1" disabled={!!editor.portfolio} required value={editor.companyId} onChange={e => setEditor({...editor, companyId:e.target.value})}><option value="">Selecione</option>{companies.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</Select></label><label className="block text-sm">Código<Input className="mt-1" maxLength={64} required value={editor.code} onChange={e => setEditor({...editor, code:e.target.value})}/></label><label className="block text-sm">Nome<Input className="mt-1" maxLength={150} required value={editor.name} onChange={e => setEditor({...editor, name:e.target.value})}/></label>{!editor.portfolio && <label className="flex gap-2 text-sm"><input checked={editor.active} type="checkbox" onChange={e => setEditor({...editor, active:e.target.checked})}/>Ativa</label>}<div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setEditor(null)}>Cancelar</Button><Button loading={create.isPending || update.isPending} type="submit">Salvar</Button></div></form></Drawer>}
  </div>;
}
