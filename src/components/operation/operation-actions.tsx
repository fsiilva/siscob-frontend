...
import { getAvailableOperationActions, operationCommandLabels } from "./operation-presenter";
...
export function OperationActions({ operation }: { operation: OperationResponse }) {
  ...
  async function submit() {
    if (!selected || !isOperationCommandValid(selected, values)) return;
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {actions.map((action) => <Button key={action} onClick={() => { mutation.reset(); setSuccess(null); setSelected(action); }} variant="secondary">{operationCommandLabels[action]}</Button>)}
      </div>
      {selected ? (
        <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <h4 className="font-semibold text-slate-950">{operationCommandLabels[selected]}</h4>
          {(selected === "assign" || selected === "transfer") && user?.role === "ADMIN" ? <Field label={selected === "assign" ? "Operador (opcional para assumir)" : "Novo operador"} onChange={(assignedOperatorId) => setValues((current) => ({ ...current, assignedOperatorId }))} value={values.assignedOperatorId} /> : null}
          ...
          <div className="flex justify-end gap-2"><Button onClick={() => setSelected(null)} variant="secondary">Voltar</Button><Button disabled={!isOperationCommandValid(selected, values)} loading={mutation.isPending} onClick={() => void submit()}>Confirmar</Button></div>
        </div>
      ) : null}
