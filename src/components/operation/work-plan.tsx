...
function WorkPlanFilterBar({ companiesQuery, filters, isAdmin, onChange }: { companiesQuery: ReturnType<typeof useCompanies>; filters: WorkPlanFilters; isAdmin: boolean; onChange<Key extends keyof WorkPlanFilters>(key: Key, value: WorkPlanFilters[Key]): void }) {
  return <section aria-label="Filtros do plano de trabalho" className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {isAdmin ? <Filter label="Tipo"><Select onChange={(event) => onChange("kind", optional(event.target.value) as WorkPlanKind | undefined)} value={filters.kind ?? ""}><option value="">Todos</option><option value="OPERATION">{workPlanLabels.operations}</option><option value="OPPORTUNITY">Oportunidades</option></Select></Filter> : null}
...
function WorkPlanCard({ featured, isAdmin, item, onCreate, onInteraction, onOpen }: { featured: boolean; isAdmin: boolean; item: WorkPlanItem; onCreate(context: CreateOperationContext): void; onInteraction(item: WorkPlanItem): void; onOpen(id: string): void }) {
  ...
  return <Card className={featured ? "border-amber-300 bg-amber-50/40 ring-1 ring-amber-200" : undefined}><CardContent className="space-y-4">
    ...
    {item.paymentPromise ? <section aria-label="Promessa de pagamento" className="min-w-0 rounded-lg border border-slate-200 bg-white p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Promessa de pagamento</p><dl className="mt-3 grid min-w-0 gap-3 text-sm sm:grid-cols-3"><Info label="Valor prometido" value={formatPaymentPromiseAmount(item.paymentPromise.promisedAmount)} /><Info label="Data prometida" value={formatPaymentPromiseDate(item.paymentPromise.promisedDate)} /><Info label="Status" value={paymentPromiseStatusLabels[item.paymentPromise.status]} /></dl></section> : null}
