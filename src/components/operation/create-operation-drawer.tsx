...
export function CreateOperationDrawer({ onClose, onCreated }: { onClose(): void; onCreated(operation: OperationResponse): void }) {
  ...
  const companies = useMemo(
    ...
    [assignedCompanies, availableCompanies, companiesQuery.data, selectedCompany],
  );
  ...
  return (
    <Drawer className="sm:w-[560px]" onClose={onClose} open title="Nova Operation">
      <form className="space-y-5 py-5" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        <Field label="Empresa"><Select disabled={companiesQuery.isLoading} required value={values.companyId} onChange={(event) => setValues((current) => changeCreateOperationCompany(current, event.target.value))}><option value="">{companiesQuery.isLoading ? "Carregando empresas..." : companies.length ? "Selecione a empresa" : "Nenhuma empresa disponível"}</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}{company.code ? ` (${company.code})` : ""}</option>)}</Select></Field>
