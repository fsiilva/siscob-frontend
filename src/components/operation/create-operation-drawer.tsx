...
export function CreateOperationDrawer({ onClose, onCreated }: { onClose(): void; onCreated(operation: OperationResponse): void }) {
  ...
  const allReceivablesQuery = useReceivables({ page: 1, pageSize: 100 });
  ...
  return (
    <Drawer className="sm:w-[560px]" onClose={onClose} open title="Nova Operation">
      <form className="space-y-5 py-5" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        <Field label="Empresa"><Select required value={values.companyId} onChange={(event) => setValues((current) => changeCreateOperationCompany(current, event.target.value))}><option value="">Selecione a empresa</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select></Field>
        ...
        <Field label="Pesquisar Customer"><Input placeholder="Nome, CPF ou CNPJ" type="search" value={customerSearch} onChange={(event) => setCustomerSearch(event.target.value)} /></Field>
        <Field label="Customer"><Select required value={values.customerId} onChange={(event) => setValues((current) => changeCreateOperationCustomer(current, event.target.value))}><option value="">Selecione o Customer</option>{customersQuery.data?.data.map((customer) => <option key={customer.id} value={customer.id}>{customer.tradeName || customer.name}</option>)}</Select></Field>
        <Field label="Receivable (opcional)"><Select disabled={!values.customerId} value={values.receivableId} onChange={(event) => update("receivableId", event.target.value)}><option value="">Sem Receivable</option>{receivables.map((receivable) => <option key={receivable.id} value={receivable.id}>{receivable.document || `Título ${receivable.id}`} — {new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(receivable.balance)}</option>)}</Select></Field>
        ...
        {portfoliosQuery.isError || customersQuery.isError || allReceivablesQuery.isError || compatibleReceivablesQuery.isError ? <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800" role="alert">Não foi possível carregar todos os dados de seleção. Tente novamente.</p> : null}
