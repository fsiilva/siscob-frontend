...
describe("collection alert schema", () => {
  ...
  it.each(["CRITICAL_WITHOUT_FOLLOW_UP", "OVERDUE_FOLLOW_UP", "DUE_TODAY", "HIGH_VALUE_WITHOUT_ACTIVE_COLLECTION"] as const)("aceita type %s", (type) => expect(collectionAlertSchema.parse({ ...alerts[0], type }).type).toBe(type));
