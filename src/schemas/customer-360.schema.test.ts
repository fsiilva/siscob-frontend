...
const customer360Response = {
  ...
  receivables: [{ id: 1, company: { id: 4, name: null }, dueDate: "2026-08-01", amount: 100, balance: 50, daysOverdue: 9, status: "OPEN" }],
...
describe("customer 360 schema", () => {
  ...
  it("aceita company.name preenchido", () => {
    const company = { id: 4, name: "Empresa Fortaleza" };
    const payload = { ...customer360Response, receivables: [{ ...customer360Response.receivables[0], company }] };
    expect(customer360Schema.parse(payload).receivables[0].company).toEqual(company);
  });
